...
# Deploy MP ingestion Cloud Run job
# This job runs daily to import MP data and committee memberships

PROJECT_ID="canada-gpt-ca"
REGION="us-central1"
JOB_NAME="mp-ingestion"
IMAGE_NAME="us-central1-docker.pkg.dev/${PROJECT_ID}/canadagpt/${JOB_NAME}"

echo "================================================================================"
echo "DEPLOYING MP INGESTION JOB"
echo "================================================================================"
...
gcloud builds submit \
  --config=packages/data-pipeline/cloudbuild-mp-ingestion.yaml \
  --project=${PROJECT_ID} \
  .
...
# Create or update Cloud Scheduler job to run daily at 6:00 AM UTC
SCHEDULER_JOB_NAME="mp-ingestion-trigger"
...
echo "✓ Cloud Scheduler configured to run daily at 6:00 AM UTC"
