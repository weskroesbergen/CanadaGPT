...
def test_enhanced_mp_ingestion():
    ...
    logger.info("=" * 80)
    logger.info("TESTING ENHANCED MP INGESTION - PRODUCTION DATABASE")
    logger.info("=" * 80)
    print()

    # Connect to production Neo4j on GCP
    ...
    try:
        ...
        logger.success(f"✅ Ingested {count} MPs to production")
        print()
        ...
        LIMIT 1
        """
        result = neo4j.run_query(query_right_hon)
        if result:
            mp = result[0]
            print()
        ...
        print()
        logger.info("=" * 80)
        logger.success("✅ ENHANCED MP INGESTION TEST COMPLETE - PRODUCTION")
        logger.info("=" * 80)
        print()
        logger.info("Summary:")
        logger.info("  - All MPs ingested successfully to production")
