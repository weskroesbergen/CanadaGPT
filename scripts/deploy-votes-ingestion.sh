...
# Deploy votes ingestion Cloud Run job
# This job runs daily at 7am to import new parliamentary votes

PROJECT_ID="canada-gpt-ca"
REGION="us-central1"
JOB_NAME="votes-ingestion"
IMAGE_NAME="us-central1-docker.pkg.dev/${PROJECT_ID}/canadagpt/${JOB_NAME}"

echo "================================================================================"
echo "DEPLOYING VOTES INGESTION JOB"
echo "================================================================================"
...
gcloud builds submit \
  --config=packages/data-pipeline/cloudbuild-votes-ingestion.yaml \
  --project=${PROJECT_ID} \
  .
...
# Create or update Cloud Scheduler job to run daily at 7am UTC
SCHEDULER_JOB_NAME="votes-ingestion-trigger"
...
echo "✓ Cloud Scheduler configured to run daily at 7:00 AM UTC"
