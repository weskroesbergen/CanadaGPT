#!/usr/bin/env python3
"""
Sample Committee Data Migration from PostgreSQL to Neo4j

Migrates a small sample of committee data to validate the approach:
- 10 committees + their full relationship graph
- All related meetings, activities, reports
- All relationships between entities

This is a test run before full migration.
"""
...
class CommitteeMigration:
    ...
    def fetch_sample_committees(self) -> List[Dict[str, Any]]:
        """Fetch sample committees from PostgreSQL"""
    ...
    def load_reports(self, reports: List[Dict[str, Any]]):
        ...
        # Note: This won't work in the sample if parent reports aren't in the sample
    ...
    def print_summary(self):
        ...
        print(f"   Committees:           {self.stats['committees']:6,}")
        print(f"   Committee Instances:  {self.stats['committee_instances']:6,}")
        print(f"   Meetings:             {self.stats['meetings']:6,}")
        print(f"   Activities:           {self.stats['activities']:6,}")
        print(f"   Activity Instances:   {self.stats['activity_instances']:6,}")
        print(f"   Reports:              {self.stats['reports']:6,}")
        print(f"   Total Relationships:  {self.stats['relationships']:6,}")
    ...
    def run(self):
        """Execute the sample migration"""
        print("=" * 80)
        print("COMMITTEE DATA SAMPLE MIGRATION")
        ...
        try:
            ...
            # Step 2: Extract data
            committees = self.fetch_sample_committees()
            ...
            print("\n" + "=" * 80)
            print("✅ SAMPLE MIGRATION COMPLETE")
            print("=" * 80)
            print("\nTo run the full migration, update SAMPLE_SIZE or remove the LIMIT clause.")
